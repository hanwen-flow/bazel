...
// Reads the server's advertised pid from the ServerInfo proto at `path` into
// *pid. Returns false if the file is missing, empty, or not a valid ServerInfo.
bool ReadRawprotoPid(const blaze_util::Path &path, int *pid) {
  string data;
  if (!blaze_util::ReadFile(path, &data) || data.empty()) {
    return false;
  }
  command_server::ServerInfo info;
